...
class BluFi {
   ...
   * @param {Object} options.onLog - 自定义LOG, 默认为null
  ...
  async init() {
    try {
      // 初始化蓝牙模块
      await this._promisify(uni.openBluetoothAdapter);
  ...
  async scanDevices(timeout = 10000, onDeviceFound = null) {
    try {
      ...
      // 开始搜寻附近的蓝牙外围设备
      await this._promisify(uni.startBluetoothDevicesDiscovery, {
      ...
      const res = await this._promisify(uni.getBluetoothDevices);
      this.logger.log('scanDevices', res);
      
      // 停止搜寻
      await this._promisify(uni.stopBluetoothDevicesDiscovery);
  ...
  async connect(deviceId) {
    try {
      ...
      // 连接设备
      await this._promisify(uni.createBLEConnection, { deviceId });
      this.logger.log('设备连接成功');
      
      // 获取服务
      const servicesRes = await this._promisify(uni.getBLEDeviceServices, { deviceId });
      ...
      // 获取特征值
      const charsRes = await this._promisify(uni.getBLEDeviceCharacteristics, {
        deviceId,
  ...
  async connect_direct(deviceId) {
    try {
      ...
      this.writeCharId = BLUFI_CHAR_WRITE_UUID;
      this.notifyCharId = BLUFI_CHAR_NOTIFY_UUID;
      
      this.sequence = -1; // 初始化序列号
      
      if(this.mtu > 0) {
          await new Promise(resolve => setTimeout(resolve, 800));
          await this._promisify(uni.setBLEMTU, { deviceId, mtu: this.mtu});
          this.logger.log('设置MTU:', this.mtu);
      }
...
async _initSecurity() {
  ...
  async disconnect() {
    if (this.deviceId) {
      try {
        if (this.isNotifying) {
          await this._promisify(uni.notifyBLECharacteristicValueChange, {
        ...
        await this._promisify(uni.closeBLEConnection, { deviceId: this.deviceId });
  ...
  async _enableNotify() {
    try {
      ...
      // 监听特征值变化
      uni.onBLECharacteristicValueChange(this._onCharacteristicValueChange.bind(this));
  ...
  _parseWifiStatusData(data) {
    try {
      ...
      const opMode = data[0]; //表示 opmode 0x00: NULL, 0x01: STA, 0x02: SoftAP, 0x03: SoftAP & STA
      const staConnStatus = data[1]; //STA 设备的连接状态。0x0 连接状态且获得IP地址，0x1 非连接状态, 0x2 正在连接状态，0x3 连接状态但未获得 IP 地址。
      const softApConnNum = data[2]; //SoftAP 的连接状态，即表示有多少 STA 设备已经连接。
  ...
  async _writeData(data) {
    ...
    try {
      await this._promisify(uni.writeBLECharacteristicValue, {
        deviceId: this.deviceId,
        serviceId: this.serviceId,
